...
## Line 7: Boundary condition 3  vy(x3) + theta(x3)*(zsc+ha/2)=d3*cos(theta0) -correct
...
B = np.matrix([[P*np.sin(theta)],
			   ...
			   [d3*np.cos(theta) + (1/(6*E*Izz))*P*np.sin(theta)*(x3-(xII))**3 - (1/(G*J))*P*(x3-(xII))*(-ha/2*np.cos(theta) + np.abs(zsc)*np.sin(theta)) ], # + (1/(E*Izz))*q  - (1/(G*J))*q Add Aero
			   [-d1*np.sin(theta)],
			   [0],
...
line6: np.array([-(1/(6*E*Izz))*(x-x1)**3-(1/(G*J))*(zsc+ha/2)*(np.abs(zsc)-ha/2),])
