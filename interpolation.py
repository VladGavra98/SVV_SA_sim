...
def interpolate(x_inp, z_inp):
    ...
    for j in range(Nz+1):
        ...
        for i in range(len(x_inp)):
            while x_inp[i]>x[i2]:
                i2+=1
            diff = x_inp[i]-x[i2]
            x_local = a_x[i2, j]*diff**3 + b_x[i2, j]*diff*diff + c_x[i2, j]*diff + d_x[i2, j]
            data_x[i,j] = x_local
    ...
    for i in range(len(x_inp)):
        j2 = 0
        for j in range(len(z_inp)):
            while z_inp[j]>z[j2]:
                j2+=1
            diff = z_inp[j]-z[j2]
            z_local = a_z[i, j2]*diff**3 + b_z[i, j2]*diff*diff + c_z[i, j2]*diff + data_x[i, j2]
            data_z[i,j] = z_local
